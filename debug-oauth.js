...
const CONFIG = {
    OIDC_WELL_KNOWN_URL: process.env.OBP_OAUTH_WELL_KNOWN_URL || 'http://127.0.0.1:9000/obp-oidc/.well-known/openid-configuration',
    ...
    try {
        ...
        if (jwks.keys && jwks.keys.length > 0) {
            jwks.keys.forEach((key, index) => {
                console.log(`   Key ${index + 1}: ${key.kty} (${key.alg || 'no alg specified'}) - ${key.kid || 'no kid'}`);
            });
        }
...
function analyzeJWT(token, oidcConfig) {
    ...
    console.log(`   Expires At: ${payload.exp ? new Date(payload.exp * 1000).toISOString() : 'Not specified'}`);
